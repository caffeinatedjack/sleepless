...
- [x] 4.12 Add integration tests for search
...
- [x] 5.2 Implement `--tag` and `--count` flags for random
- [x] 5.3 Implement `regimen note report` command
- [x] 5.4 Implement `--days` flag for report
...
- [x] 6.7 Support `--passphrase-stdin` flag and interactive prompt (confirm on encrypt)
- [x] 6.8 Encrypt eligible file types: `.md` and `.json` (skip `.git/`, skip symlinks)
...
- [x] 7.4 Implement `regimen note add --template <name>` integration
